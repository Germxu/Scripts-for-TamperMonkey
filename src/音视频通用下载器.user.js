...
(function () {
	...
	(function () {
		...
		function play() {
			...
			if (link.indexOf("blob") != 0) {
				window.top.postMessage(
					Array("audioVideoCapturer", link, windowID, "link"),
					"*"
				);
			}
		...
		HTMLAudioElement.prototype.play.toString =
			HTMLAudioElement.prototype.play.toString.bind(playOriginal);
	})();
	(function () {
		...
		HTMLVideoElement.prototype.play.toString =
			HTMLVideoElement.prototype.play.toString.bind(playOriginal);
	})();
	(function () {
		...
		class MediaSourcE extends MediaSourceOriginal {
			constructor() {
				...
				window.top.postMessage(
					Array("audioVideoCapturer", id, windowID, "blob"),
					"*"
				);
			}
		}
	...
	(function () {
		...
		MediaSource.prototype.addSourceBuffer.toString =
			MediaSource.prototype.addSourceBuffer.toString.bind(
				addSourceBufferOriginal
			);
	})();
	(function () {
		...
		SourceBuffer.prototype.appendBuffer.toString =
			SourceBuffer.prototype.appendBuffer.toString.bind(appendBufferOriginal);
	})();
	...
	waitForBody(function () {
		...
		iframe.style.zIndex = 999_999_999_999_999;
		iframe.style.zIndex = 214_748_3647;
	...
	if (window.top == window) {
					...
					if (iframe2.contentDocument) {
						...
						if (event.data[3] == "blob") {
							elt.outerHTML =
								'<a id="' +
								id +
								'" style="text-decoration: underline; cursor: pointer;">Blob: ' +
								event.data[1].slice(0, 20) +
								"</a>\r\n";
							iframe2.contentDocument.getElementById(id).onclick = function () {
								window.postMessage(
									[
										"audioVideoCapturerDownloadBlob",
										event.data[2],
										event.data[1],
									],
									"*"
		...
		let onShowRequest = function onShowRequest(event) {
			if (
				!(
					event.data &&
					event.data.audioVideoCapturerShow &&
					event.data.audioVideoCapturerShow.show
				)
			) {
				return;
			}
	...
	if (
		true ||
		location.href.toLocaleLowerCase().indexOf("xue.taobao.com") != -1 ||
		location.href.toLocaleLowerCase().indexOf("edu.taobao.com") != -1 ||
		location.href.toLocaleLowerCase().indexOf("kuwo") != -1 ||
